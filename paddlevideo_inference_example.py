...
def cal_video(args_input):
    ...
    while vid.isOpened():
        ...
        erode_sum = np.sum(erode)
        diffsum = np.sum(diff)
        wrt.write("{}, {}, {}\n".format(erode_sum, diffsum, 0))
        prev_gray = gray
        count = count + 1
    vid.release()
...
def extract_feature_from_csv_and_segment(segment_list: list,csv_file: str,windows_size=60, step=60,vid=None,src=None):
    """
    不切片
    Args:
...
def step2(args_input):
    ...
    segment = filter(pass_points, inter, intra, diff_df) 
    X_df, location = extract_feature_from_csv_and_segment(segment, dst_csv, windows_size=60, step=60,vid=vid, src=src)
    vid.release()
    # 信号分类算法
...
def cpu_process(list_args, queue: Queue):
    """
    ...
    for i in list_args:
        ...
        if (src.parent/(src.stem+"_location.csv")).exists():
            ...
            location_pd.to_csv(video_filename.parent/(video_filename.stem+"_location_all.csv"))
        queue.put([i, video_filename])
        

def gpu_process(list_args, queue: Queue):
    from ppvideo import PaddleVideo
    model_str = "/home/nvidia/sleap-mouse/ppTSM20221208/ppTSM_mouse_20220613"
    ...
    while True:
        ...
        if not queue.empty():
            ...
            if video_filename == '':
                continue
            if (video_filename.parent/(video_filename.stem+"_location.csv")).exists():
...
def main_split(args: list = None):
    ...
    queue = Queue()
    cpu_part = Process(target=cpu_process, args=(list_args, queue))
    gpu_part = Process(target=gpu_process, args=(queue))
    ...
    with tqdm(total=len(list_args),) as t:
        for location in p.imap(step2, list_args):
            ...
            if (src.parent/(src.stem+"_location.csv")).exists():
                ...
                location_pd = predict_signal(location[0], location[1],clf)
                if location_pd.shape[0] == 0:
                    continue
                video_filename = Path(location_pd.loc[0,'filename'])
